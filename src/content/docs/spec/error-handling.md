...
| `ERR-AUTH-010` | 400 | リセットトークンが無効または期限切れ |
| `ERR-WF-ATT-001` | 400 | 許可されていないファイル形式 |
| `ERR-WF-ATT-002` | 400 | ファイル未指定 |
...
- エラー集約ツール（Sentry 等）の導入は将来フェーズで検討

> [!NOTE]
> 429 レート制限は `@nestjs/throttler` で実装済み。詳細は[権限と認可](../authz/)ページの「レート制限」セクションを参照。
