...
PostgreSQL と SQLite の両対応をサポートするため、型マッピングには以下のルールを適用します。
...
| `numeric(12,2)` | `Decimal @db.Decimal(12,2)` (PG) or `Float` (SQLite) |
| `integer` | `Int` |
| `boolean` | `Boolean` |
| `jsonb` | `Json` |
| `bigint` | `BigInt` |
...
> [!NOTE] マルチDB対応について
> Prisma では `provider = ["sqlite", "postgresql"]` といった複数プロバイダー指定が廃止傾向にあるため、実際には環境変数による切替や、PostgreSQL と SQLite 用に `schema.prisma` を出し分ける・もしくは共通で扱える型（`Float` や String の Date 等）を採用する運用を想定します。本仕様では PostgreSQL 向けのネイティブ型アノテーションを明記します。
...
PostgreSQL の `CHECK` 制約などで表現されていた値の制限は、Prisma の `enum` で定義します。（※SQLite環境で運用する場合は、DB側は String にしつつ TypeScript レベルでのバリデーションにフォールバックさせます）
...
1. **基本フロー**
   ...
   ```
2. **既存DBとの互換性・対応**
   Supabase を引き続き利用する場合は、ローカルの Supabase DB に対して `prisma migrate` を適用することも可能です。その際、Auth や Storage に関連するネイティブな Trigger や Policy 等の資産が初期化されないよう、併用する設定方針を適宜評価してください。
3. **複数DBプロバイダ（PostgreSQL / SQLite）対応の留意点**
   環境に応じて出力する Prisma Client や schema.prisma の向き先を分離・生成する npm scripts（例: `npm run generate:sqlite`）を用意し、CICD 上で各プロジェクトの要求に応じたプロバイダーを引き込むワークフローを組成します。
